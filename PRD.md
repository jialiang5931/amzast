...
### 4.4 MetaSpy 实时查询 (Real-time Query)
#### 4.4.1 功能概述
允许用户通过输入亚马逊广告库 URL 实时抓取广告数据，以表格形式快速预览和分析竞品广告，无需存储到本地数据库。

#### 4.4.2 核心展示字段
1.  **素材预览 (Material Preview)**: 支持图片和视频预览。视频类型带有播放图标标识。
2.  **资料库编号 (Ad Archive ID)**: 显示唯一 ID，支持一键复制代码，点击可直接跳转至 Meta 广告库详情页。
3.  **公共主页 (Page Name)**: 显示广告发布者的主页名称，支持品牌主与共创者信息显示。
4.  **投放时间/时长 (Dates/Duration)**: 自动计算投放时长（天），支持居中对齐展示。
5.  **广告标题与摘要 (Title & Body)**: 
    - 采用 `line-clamp-2` 策略，支持最多显示两行文字。
    - 固定列宽（标题 320px, 摘要 448px），确保横向滑动时布局稳定。
6.  **跳转页 (Jump Page)**: 智能识别 CTA 类型（如 SHOP_NOW, LEARN_MORE），并提供对应的落地页/中间页跳转链接。

#### 4.4.3 表格交互与性能优化
1.  **全局冻结列 (Frozen Columns)**: 前 6 列（序号、编号、素材、主页、日期、时长）在横向滑动时保持固定，便于核心数据比对。
2.  **视觉辅助**:
    - **不透明斑马纹**: 采用实色 `bg-slate-50` 进行隔行渲染，彻底消除滑动时的“文字穿透”干扰。
    - **增强表头**: 表头文字加粗并增大至 `14px`，重点数据单元格同步增大。
    - **悬停反馈**: 行悬停背景加深至 `bg-slate-100`。
3.  **零延迟响应**: 纯前端列宽计算与偏移校准，确保缩放及滚动时的丝滑体验。
...
- [x] FBM 配送方式显示逻辑
- [x] MetaSpy 实时查询模块开发 (NEW)
- [x] 表格布局优化（冻结列/斑马纹/自适应固定宽）
